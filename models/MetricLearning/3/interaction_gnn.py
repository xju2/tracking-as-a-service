...
class InteractionGNN(EdgeClassifierStage):
    ...
    def forward(self, batch, **kwargs):
        ...
        if self.training and self.hparams.get("checkpointing", False):
...
class InteractionGNNWithPyG(EdgeClassifierStage):
    ...
    def forward(self, batch, **kwargs):
        ...
        if self.training and self.hparams.get("checkpointing", False):
            ...
            for i in range(self.hparams["n_graph_iters"]):
                conv = partial(checkpoint, self.convs[i], use_reentrant=False)
                x, e = conv(edge_index, x, e)
        else:
            x = self.node_encoder(x)
            e = self.edge_encoder(torch.cat([x[start], x[end]], dim=1))
            for i in range(self.hparams["n_graph_iters"]):
                x, e = self.convs[i](edge_index, x, e)
...
class InteractionGNN2(EdgeClassifierStage):
    ...
    def configure_model_my(self):
        """Compile only the model components for optimized inference."""
        torch._dynamo.reset()
        # ✅ Compile individual submodules that require speed-up
        self.node_encoder = torch.compile(self.node_encoder, dynamic=True, mode="max-autotune")
        self.edge_encoder = torch.compile(self.edge_encoder, dynamic=True, mode="max-autotune")
        
        if isinstance(self.edge_network, nn.ModuleList):
            self.edge_network = nn.ModuleList(
                [torch.compile(layer, dynamic=True, mode="max-autotune") for layer in self.edge_network]
            )
        else:
            self.edge_network = torch.compile(self.edge_network, dynamic=True, mode="max-autotune")

        if isinstance(self.node_network, nn.ModuleList):
            self.node_network = nn.ModuleList(
                [torch.compile(layer, dynamic=True, mode="max-autotune") for layer in self.node_network]
            )
        else:
            self.node_network = torch.compile(self.node_network, dynamic=True, mode="max-autotune")

        self.edge_decoder = torch.compile(self.edge_decoder, dynamic=True, mode="max-autotune")
        self.edge_output_transform = torch.compile(self.edge_output_transform, dynamic=True, mode="max-autotune")
        
        print("✅ Model components successfully compiled for inference speed-up")
